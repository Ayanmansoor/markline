...
import { getCollectionBaseOnGender, getAllBanner, getProductBaseOnCollection, getAllCollectionWithProducts, getAllProductsbygender, } from '@/Supabase/SupabaseApi'
import { useParams } from 'next/navigation'
import { useQuery } from 'react-query'
import { CldImage } from 'next-cloudinary';
import Link from 'next/link'
import ProductFilter from '../Common/ProductFilter'
import { Colors, NewProductProps, ProductVariant, Sizes } from '@/types/interfaces'
...
function GenderPage() {
    ...
    }, [getallproductbaseongender, productRangevalue, productslug, selectColorAndSizes]);
            ...
            <section className="w-full min-h-[300px] relative  gap-5  ">
                <Breadcrumb className='w-full relative  md:px-5 lg:px-10'>
                    <BreadcrumbList className='w-full relative h-auto flex items-center py-5 rounded-lg px-3 '>
                        ...
                        <BreadcrumbSeparator />
                        <BreadcrumbItem>
                            <BreadcrumbLink href={`/collections`} className=' text-sm sm:text-base md:text-xl lg:text-2xl text-primary cursor-pointer'>collection</BreadcrumbLink>
                        </BreadcrumbItem>
                        <BreadcrumbSeparator />
                        <BreadcrumbItem>
                            <BreadcrumbLink href={`/collections/${nslug}`} className=' text-sm sm:text-base md:text-xl lg:text-2xl text-primary cursor-pointer'>{nslug}</BreadcrumbLink>
                        </BreadcrumbItem>
                    </BreadcrumbList>
                </Breadcrumb>
