...
function AddressSection({ address, handleperform }: addressprops) {
  ...
  return (
    <section className='flex flex-col gap-5 p-4 bg-white h-full'>
      <AddAdressSheet handleperform={handleperform}>
        <button className='justify-self-end px-10 w-fit bg-gray-200  py-2 rounded-full text-sm font-medium border border-gray-200  text-primary'>
          Add Address
        </button>
      </AddAdressSheet>
      <section className=' w-full  bg-white relative grid grid-cols-1  sm:grid-cols-2 lg:grid-cols-3 '>
        {
          address &&
