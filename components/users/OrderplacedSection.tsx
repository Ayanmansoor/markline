...
function OrderplacedSection({ orders, handleperform }: ordersprops) {
  ...
  return (
    <section className='w-full relative h-full  flex flex-col max-h-[500px] overflow-y-auto '>
      <div className=' sticky z-30 top-0  hidden bg-white md:grid grid-cols-[1fr_1fr_1fr_1fr_1fr_auto] gap-2  w-full h-fit  py-5 border px-10 '>
      ...
      <section className=' grid grid-cols-2 md:flex flex-col gap-3   relative  '>
        {
          ...
          (orders?.length > 0) ?
            orders.map((items, index) => (
              <Link href={`user/order/${items?.id}`} className=' flex flex-col  md:grid md:grid-cols-[1fr_1fr_1fr_1fr_1fr_auto]   gap-1 md:gap-5  lg:gap-2 px-2 lg:px-5  md:px-10  border  py-2  lg:py-3 w-full relative border-gray-200 items-start  md:items-center ' key={index}>
            ...
            :
            <div className='flex items-center h-full justify-center py-3 md:py-5  px-3  flex-col w-full'>
              <p className=' text-base lg:text-lg  text-center font-medium text-primary mb-3 '>
                Not Have Any Orders
              </p>
              <Link href={"/collections"} className='text-sm font-medium bg-primary  text-white border border-gray-300 px-5 rounded-md cursor-pointer py-2   w-fit text-center w-fit '>Buy Now</Link>
            </div>
        }
