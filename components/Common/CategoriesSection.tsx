        ...
        <section className='w-full relative  h-auto items-start gap-5  py-10 lg:py-20  flex flex-col  px-5 lg:px-10 xl:px-20 2xl:px-40  '>
            <div className='w-full relative h-auto flex-col-reverse sm:flex-row flex items-start justify-between gap-0 sm:gap-2'>
                <span className='flex flex-col gap-1 '>
                    {
                        isH1 ?
                            <h1 className='     text-lg   md:text-2xl xl:text-3xl self-start  font-semibold   w-fit  uppercase   '>{title}</h1>
                            :
                            <h2 className='   text-lg md:text-2xl xl:text-3xl  font-semibold     uppercase self-start   w-fit  '>{title}</h2>
                    }
                    {
                        subtitle &&
                ...
                {
                    url &&
                    <Link href={`/${url}`} className='text-[12px] sm:text-sm xl:text-lg w-fit self-end  font-medium text-foreground flex items-center text-[#128C7E] gap-[2px] sm:gap-1 capitalize' >{urltext} <GoArrowUpRight className='text-[12px] sm:text-[20px] ' /> </Link>
                }
            </div>
