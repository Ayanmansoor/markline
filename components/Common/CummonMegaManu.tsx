...
const MENU_ITEMS = [
    { label: "Best Selling", icon: true },
    { label: "Trending", icon: true },
    { label: "All Products", icon: true },
]
                ...
                <NavigationMenuItem>
                    ...
                    <NavigationMenuContent className="min-w-[800px] left-0 h-[400px] relative p-3 rounded-lg border border-gray-200 grid grid-cols-[.5fr_2fr] gap-3">
                        <aside className="w-full bg-white border-r border-border  overflow-y-auto">
                            <div className="mb-8">
                                <h3 className="text-base font-semibold text-primary mb-4">Featured:</h3>
                        ...
                        <main className="flex flex-col gap-4 overflow-y-auto">
                            ...
                            <div className="grid grid-cols-5 gap-4">
                                {collections.data && collections.data.length > 0 ? (
                                    collections.data
                                        .filter((col) => col?.gender?.toLowerCase() === urlProps.toLowerCase()).slice(0, 6)
                                        .map((collec, index) => {
                                            const images = collec.image_urls?.map((obj: any, index: number) => JSON.parse(obj))[0]
                                            console.log(images, "dsfkjshdflksdjfhlsdkfzDs")
                                            return (
                                                <Link href={`/collections/${collec.slug}`} key={collec.id} className="flex flex-col items-center">
                                                    ...
                                                    <p className="text-sm font-medium text-center text-foreground">{collec.name}</p>
                                                </Link>
                                            )
                                        })
                                ) : (
